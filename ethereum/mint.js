...
issueByPart = async (partition) => {
  const accounts = await web3.eth.getAccounts();
	await booking.methods.issueByPartition(partition, accounts[0], issuanceAmount, VALID_CERTIFICATE);
...
function getId(timeslot_index, company_index, roomnumber_index, status_index) {
  return ''.concat(timeslot_index, company_index, roomnumber_index, status_index);
}

function getPartition(timeslot_index, company_index, roomnumber_index, status_index) {
